 ...
 */
export async function getTopCandidates({ limit = 10, screeningOverrides = null, explorationMode = false } = {}) {
  ...
  // D4: Now supports exploration mode via separate thresholds in gmgn-config.json
  const discovery = source === "gmgn"
    ? await discoverGmgnPools({ limit: Math.max(limit, config.gmgn.enrichLimit || 20), explorationMode })
