...
const CONFIG_VALIDATORS = {
  ...
  trailingVolMultiplier:  num(0, 10),
  trailingVolPivot:       num(0, 1000),
  trailingVolMaxScale:    num(0, 1000),
  trailingMinTriggerPct:  num(0, 10000),
  trailingMaxTriggerPct:  num(0, 10000),
  trailingMinDropPct:     num(0, 100),
  trailingMaxDropPct:     num(0, 100),
...
const toolMap = {
  ...
  update_config: ({ changes, reason = "" }) => {
    ...
    const CONFIG_MAP = {
      ...
      trailingVolMultiplier: ["management", "trailingVolMultiplier"],
      trailingVolPivot: ["management", "trailingVolPivot"],
      trailingVolMaxScale: ["management", "trailingVolMaxScale"],
      trailingMinTriggerPct: ["management", "trailingMinTriggerPct"],
      trailingMaxTriggerPct: ["management", "trailingMaxTriggerPct"],
      trailingMinDropPct: ["management", "trailingMinDropPct"],
  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        notifySwap({ inputSymbol: args.input_mint?.slice(0, 8), outputSymbol: args.output_mint === "So11111111111111111111111111111111111111112" || args.output_mint === "SOL" ? "SOL" : args.output_mint?.slice(0, 8), amountIn: result.amount_in, amountOut: result.amount_out, tx: result.tx }).catch((err) => log("notify_warn", err.message));
      } else if (name === "deploy_position") {
        notifyDeploy({ pair: result.pool_name || args.pool_name || args.pool_address?.slice(0, 8), amountSol: args.amount_y ?? args.amount_sol ?? 0, position: result.position, tx: result.txs?.[0] ?? result.tx, priceRange: result.price_range, rangeCoverage: result.range_coverage, binStep: result.bin_step, baseFee: result.base_fee }).catch((err) => log("notify_warn", err.message));
      } else if (name === "close_position") {
        notifyClose({ pair: result.pool_name || args.position_address?.slice(0, 8), pnlUsd: result.pnl_usd ?? 0, pnlPct: result.pnl_pct ?? 0, reason: args.reason }).catch((err) => log("notify_warn", err.message));
