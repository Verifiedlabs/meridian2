...
export async function discoverGmgnPools({ limit = 10, explorationMode = false } = {}) {
  // Use exploration thresholds when in exploration mode, otherwise use normal thresholds
  const g = explorationMode && config.gmgn.exploration 
    ? { ...config.gmgn, ...config.gmgn.exploration }
    : config.gmgn;
  ...
  if (explorationMode) {
    log("gmgn", `Using EXPLORATION thresholds (relaxed filters)`);
  }
