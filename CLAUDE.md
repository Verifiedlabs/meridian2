...
- `evolveThresholds()` now evolves `maxVolatility` (default 5.0 in `config.screening`) and `minFeeActiveTvlRatio` correctly. Persisted to `user-config.json` and re-applied via `reloadScreeningThresholds()`.
